...
struct node * insert_print_tree(int value, struct node * t) 
{
  struct node * print_new_node;
    
  if (t == NULL) 
  {
    print_new_node = (struct node *) malloc (sizeof (struct node));
    if (print_new_node == NULL) 
    {
	    return t;
    }

    print_new_node->data = value;

	  print_new_node->left = print_new_node->right = NULL;
	  return print_new_node;
  }
  
  if (value < t->data) 
  {
    t->left = insert_print_tree(value, t->left);
  } 
  else if (value > t->data) 
  {
	  t->right = insert_print_tree(value, t->right);
  } 
  else 
  { 
    //duplicate, ignore it
...
struct node * delete(int value, struct node * t) 
{
